...
port 5095 if not running, executes the 20-target corpus through
``FieldValidationRunner``, prints a one-line summary, writes the JSON
report, and exits non-zero if the gate is not met (TPR ≥ 0.90 AND
FPR == 0).
...
Usage::
    ...
    python ../../scripts/run_open_redirect_field_validation.py
"""
...
FIXTURE = ORACLE_DIR / "tests" / "fixtures" / "open_redirect_targets.json"
LAB_APP = ORACLE_DIR / "tests" / "fixtures" / "open_redirect_lab" / "app.py"
REPORT = ORACLE_DIR / "tests" / "reports" / "phase2_open_redirect_tpr_fpr.json"
LAB_HOST = "127.0.0.1"
LAB_PORT = 5095
...
def main() -> int:
    ...
    try:
        ...
        print(
            f"Phase 2 Open Redirect field validation: "
