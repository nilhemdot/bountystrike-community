...
> **Most credible incumbent threat (v6 re-rank, 2026-05-27):** **Bugcrowd** — post-Mayhem/ForAllSecure acquisition (Nov 4 2025), now holding coverage-guided fuzzing + symbolic execution with David Brumley as Chief AI/Science Officer. A $1B+ bug-bounty platform on an AEV trajectory is a more credible competitive threat than any single Tier-1 startup below. See the Bugcrowd row in Tier 2. *(Note: if Anthropic releases Mythos publicly at the $25/$125 per-MTok ceiling before the ~July 2026 Glasswing window closes, the AEV category reprices around that token cost — keep the cost model fast to re-derive.)*

### Tier 1 — well‑funded autonomous pentesting startups

| Company | Funding (latest) | Architecture signature | Why it matters | Weakness |
|---|---|---|---|---|
| **XBOW** | $237M total; $120M Series C, Mar 2026 ($1B+) | Thousands of short‑lived parallel agents; deterministic exploit verification (e.g., headless browser to confirm XSS payload executed) separated from AI exploration | Topped HackerOne US leaderboard June 2025; **Pentest On‑Demand GA Nov 2025** compresses 35–100‑day pentests into hours | ~25% Informative/N/A on H1 submissions *(industry estimate, not vendor-confirmed — per v6)*; once removed from a program for being an automated scanner |
...
| **Bugcrowd** ⭐ **most credible incumbent threat** | **Acquired Mayhem Security (ForAllSecure)** Nov 4 2025; David Brumley becomes Chief AI/Science Officer | Bugcrowd now owns DARPA‑Cyber‑Grand‑Challenge‑era coverage‑guided fuzzing + symbolic execution tech (claims 2× more bugs than fuzzing alone, zero FPs via proof‑of‑vulnerability). **The dual-product play competes directly with Bugcrowd's AEV trajectory** — re-ranked above the Tier-1 startups per v6 pressure-test |
...
Pricing as of April 25, 2026 (verified per vendor pricing pages). Token assumptions: full triage scan ≈ 500K input + 80K output; recon analysis ≈ 200K input + 30K output; report drafting ≈ 30K input + 15K output. The "$/scan" column is full‑triage cost. DeepSeek rows corrected to official API pricing (DeepSeek official API pricing, verified v6 2026-05-27).

| Model | Input $/M | Output $/M | $/scan | Solo $5? | SaaS $50? | Best for |
|---|---|---|---|---|---|---|
| **DeepSeek V4‑Flash** | 0.14 (cache 0.0028) | 0.28 | **$0.07** cached | ✅ | ✅ | Bulk triage |
| **DeepSeek V4‑Pro** | 1.74 (currently 75% off) | 3.48 | **$1.15** | ✅ | ✅ | Cheap reasoning |
| **DeepSeek V3.2** | 0.14 | 0.28 | $0.09 | ✅ | ✅ | Triage fallback |
