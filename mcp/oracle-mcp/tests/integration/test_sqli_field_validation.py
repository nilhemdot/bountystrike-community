...
brings up the local Flask lab on port 5096, polls /healthz until ready,
runs the FieldValidationRunner against the committed
``tests/fixtures/sqli_targets.json`` corpus, and asserts the Phase 2
exit gate (TPR ≥ 0.90 AND FPR == 0).

Why opt-in:
  - 40 oracle calls with time-delay testing = significant wall clock time.
  - The unit suite already covers the runner's pure logic.
  - Welch t-test timing verification requires multiple samples per target.

To run::

    cd mcp/oracle-mcp
    uv pip install -e .[dev] flask
    RUN_SQLI_FIELD_VALIDATION=1 pytest tests/integration/test_sqli_field_validation.py -v

The harness writes the resulting report to
``tests/reports/phase2_sqli_tpr_fpr.json`` next to the fixture so a
...
FIXTURE_PATH = (
    Path(__file__).resolve().parents[1] / "fixtures" / "sqli_targets.json"
)
LAB_APP = (
    Path(__file__).resolve().parents[1] / "fixtures" / "sqli_lab" / "app.py"
)
REPORT_PATH = (
    Path(__file__).resolve().parents[1] / "reports" / "phase2_sqli_tpr_fpr.json"
)
LAB_HOST = "127.0.0.1"
LAB_PORT = 5096
HEALTH_URL = f"http://{LAB_HOST}:{LAB_PORT}/healthz"

OPT_IN_ENV = "RUN_SQLI_FIELD_VALIDATION"

pytestmark = pytest.mark.skipif(
    os.environ.get(OPT_IN_ENV) != "1",
    reason=f"opt-in: set {OPT_IN_ENV}=1 to run Phase 2 W9-10 integration",
)
...
def lab_process():
    ...
    Skipped automatically if port 5096 is already bound — that's the
    docker-compose case (``docker compose up sqli-lab``). Either way the
...
def test_sqli_field_validation_meets_phase2_gate(lab_process) -> None:
    ...
    assert report.passes_exit_criterion(min_tpr=0.90, max_fpr=0.0), (
        f"Phase 2 W9-10 gate failed: tpr={report.tpr:.3f} (need ≥0.90), "
